                ...
                loadHandler: function (tab) {
                    var ajaxSettings = {
                        ...
                        success: function (data) {
                            _this.handleReponseFromThridParty(tab, callback, data);
                        },
                        error: function (xhr, errorText, errorObject) {
                            _this.handleReponseFromThridParty(tab, callback, { error: errorText });
                        }
                    };
                    if (options.header) {
                        ajaxSettings.headers = {};
                        for (var i = 0; i < options.header.length; i++) {
                            ajaxSettings[options.header[i].key] = options.header[i].value;
                        }
                    }
                    ...
                    _this.cstimeout = setTimeout(function () {
                        _this.handleReponseFromThridParty(tab, callback, { error: 'timeout' });
                    }, 10000);
                }
            });
        };
        ProvisioningService.prototype.handleReponseFromThridParty = function (tab, callback, result) {
