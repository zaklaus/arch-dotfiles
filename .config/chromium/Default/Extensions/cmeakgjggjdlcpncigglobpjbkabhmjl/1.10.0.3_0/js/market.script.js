...
var ShowBookmarks = function () {
    ...
    if (bookmarkeditems) {
        ...
        $J.each(bookmarkeditems, function (idex, item) {
            ...
            var itemLink = window.location.protocol + "//steamcommunity.com/market/priceoverview/?appid=" + item.appid
...
var UpdateBookmarksPrices = function () {
    ...
    $J('#tabContentsMyBookmarks .my_listing_section .market_listing_row.market_recent_listing_row').each(function (idx) {
        var $div = $J(this);
        var data = $div.data();
        var itemLink = window.location.protocol + '//steamcommunity.com/market/priceoverview/?appid=' + data.appid
            + '&country=' + countryCode + '&currency=' + currencyId
...
var GetAppList = function () {
    $J.ajax({
        url: 'http://steamcommunity.com/market/myhistory/render/?start=' + startPage + '&count=' + window.historypagesize,
        method: 'get'
    }).done(function (res) {
        assetList = [];
