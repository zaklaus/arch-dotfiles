...
function SortInventory() {
  if ($J('#Lnk_SortItems').data('asc')) {
    $J('#Lnk_SortItems').find('.item_market_action_button_contents').html('▲ Sort items');
...
var SortItem = function (asc) {
    ...
    var curInventoryPage;
    $J(' .inventory_page[style != "display: none;"] ').each(function () {
      if ($J(this).parent().css('display') !== 'none') {
        curInventoryPage = $J(this);
        return true;
      }
    });
    if (!userInventory.length) userInventory = $J(' .inventory_page .itemHolder:has( .app730 ) ');
    var curPage = parseInt($J('#pagecontrol_cur').text() || 1, 10);
    var sortedElems;
    if (lastSort !== null) {
      sortedElems = userInventory.sort(sortFunc).slice((curPage-1)*25, curPage * 25);
    } else {
      sortedElems = userInventory.slice((curPage-1)*25, curPage * 25);
    }
    curInventoryPage.find(' .itemHolder:has( .app730 ) ').remove();
    sortedElems.each(function (idx, el) {
      elObj = $J(el).find('.item')[0].rgItem;
      if (!$J(el).find(' img ').length) {
        $J(el).find(' a ').after('<img src="http://steamcommunity-a.akamaihd.net/economy/image/' + elObj.icon_url + '/96fx96f" >')
      }
    });
    sortedElems.prependTo(curInventoryPage);
};
...
var ModifySellingFuntions = function () {
    ...
    if (window.inventoryprice) {
        $J('#Lnk_SendGifts').after('<div class="price-holder"><div id="total-price" class="total-inventory-price">Total price</div><a href="#" class="lnk_inventory_price">Get price</a></div>');
    }
    ...
    $J('#Lnk_Sellall').click(function () {
        ...
        $J('div[id^="inventory_"]:visible.inventory_ctn').each(function (i) {
            ...
            $visiblePage.find('div[id^="item"].item:visible').each(function (i) {
    ...
    $J('#Lnk_SendGifts').click(function () {
        ...
        $J('.selectedSell').each(function () {
            ...
            if (rgItem.appid == 753 && rgItem.contextid == 1) {
                ...
                if (rgItem.owner_descriptions && Array.isArray(rgItem.owner_descriptions)) {
                    var re = new RegExp('<a href=.*://steamcommunity.com/.* data-miniprofile="\\d+">.*</a>');  // ссылка на профиль получателя
                    isSent = rgItem.owner_descriptions.some(function (desc) {
...
setTimeout(function () {
    ...
    $J('body').on('click', '#bt_quick_sell', function () {
        var itemsAmount = $J('.queue-item-container').length;
        var currentItem = 0;
        var time = 200;
    ...
    if (window.extprice) {
        ...
        var sortButton = '<a href="javascript:SortInventory();" id="Lnk_SortItems" style="margin-left: 12px" class="item_market_action_button item_market_action_button_green">' +
          ...
          '<span class="item_market_action_button_contents" data-lang="sortitem">Sort items</span>' +
        ...
        if (g_bViewingOwnProfile) {
          ...
          $J('#filter_options').after('<div>' + sortButton + '</div>')
        }
    }
    ...
    if ($J('#inventory_pagecontrols').length) {
        $J('#inventory_pagecontrols #pagebtn_previous').click(function () {
        ...
        $J('#inventory_pagecontrols #pagebtn_next').click(function () {
            if (!$J(this).hasClass('disabled')) {
              setTimeout(function () {
                SortItem(lastSort);
              }, 300);
    ...
    $J('.games_list_tab').click(function () {
      if ($J(this).attr('id') == 'inventory_link_730') {
        ...
        $J('.itemHolder:has(.app730)').each(function () {
          $J(this).remove();
        })
      }
    });
    ...
    if (ExternalPrices[appid]) {
        ...
        $J.each(ExternalPrices[appid].apis, function (idx, el) {
            if (this.api && this.api.GetPrices) {
                ...
                if (['csgofast.com', 'backpack.tf'].indexOf(opt[0].innerText) !== -1) {
...
var GetAllInventoryPrice = function (inventar) {
    inventar.find('div[id^="item"].item').each(function () {
        var dataEconomyItem = $J(this).attr('id').substr(4).split('_');
...
var CheckItem = function (prefix, item, owner) {
    ...
    var sItem = item || g_ActiveInventory.selectedItem;
...
function showNonTradableItems() {
    ...
    function getGameInfo() {
        ...
        $J.ajax({
            url: 'https://steamcommunity.com/id/nook_3000/inventory/json/' + gameData[counter].id + '/' + gameData[counter].param
        }).done(function (res) {
            if (res.success) {
